...
const DynamicContainer = ({ className, children }: { className?: string; children?: React.ReactNode }) => {
  const willChange = useWillChange()
  ...
  return (
    <motion.div
      ...
      transition={{ type: "spring" as const, stiffness, damping, duration: isSizeChanged ? 0.5 : 0.8 }}
...
const DynamicDiv = ({ className, children }: { className?: string; children?: React.ReactNode }) => {
  const { state } = useDynamicIslandSize()
  const { size, previousSize } = state
  const willChange = useWillChange()
...
const DynamicTitle = ({ className, children }: { className: string; children: React.ReactNode }) => {
  const { state } = useDynamicIslandSize()
  const { size, previousSize } = state
  const willChange = useWillChange()
...
const DynamicDescription = ({ className, children }: { className: string; children: React.ReactNode }) => {
  const { state } = useDynamicIslandSize()
  const { size, previousSize } = state
  const willChange = useWillChange()
